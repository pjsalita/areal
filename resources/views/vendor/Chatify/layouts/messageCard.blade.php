...
@if($viewType == 'default')
    ...
    <div class="message-card" data-id="{{ $id }}">
        <p>{!! ($message == null && $attachment != null && (@$attachment[2] != 'file' || substr(@$attachment[0], strrpos(@$attachment[0], '.') + 1) === 'mp4')) ? $attachment[1] : nl2br($message) !!}
            <sub title="{{ $fullTime }}">{{ $time }}</sub>
            {{-- If attachment is a file --}}
            @if(@$attachment[2] == 'file' && substr(@$attachment[0], strrpos(@$attachment[0], '.') + 1) !== 'mp4')
    ...
    <div>
        <div class="message-card">
            <div class="image-file chat-image" style="width: 250px; height: 150px;">
                <video style="width: 250px; height: 150px;" src="{{ Storage::url(config('chatify.attachments.folder') . "/{$attachment[0]}") }}" controls></video>
            </div>
        </div>
    </div>
    @endif
